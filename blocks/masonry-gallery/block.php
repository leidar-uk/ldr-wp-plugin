...
class Masonry_Gallery_Block {
    ...
    public function ldr_register_acf_block() {
        ...
        if( function_exists( 'acf_register_block_type' ) ) {
            ...
            acf_register_block_type( [
                ...
                'title'             => __( 'Masonry Gallery', 'ldr' ),
                'description'       => __( 'A masonry grid gallery.', 'ldr' ),
                'render_template'   => $this->block_dir . 'template.php',
                'category'          => 'custom',
                'icon'              => file_get_contents( $this->block_dir . 'images/masonry-gallery.svg' ),
                'keywords'          => ['masonry', 'gallery'],
                'enqueue_assets'    => function() {
                    wp_enqueue_style( 'masonry-gallery', $this->block_url . 'style.min.css', [], filemtime( $this->block_dir . 'style.min.css' ) );
                    wp_enqueue_script( 'masonry-gallery', $this->block_url . 'script.min.js', [], filemtime( $this->block_dir . 'script.min.js' ), true );
                }
            ] );
    ...
    public function ldr_register_field_group() {
        ...
        if( function_exists( 'acf_add_local_field_group' ) ) {
            ...
            acf_add_local_field_group( [
                'key' => 'group_masonry_gallery_settings',
                'title' => __( 'Masonry gallery', 'ldr' ),
                'fields' => $fields,
                'location' => [
                    [
    ...
    // Gallery fields
    protected function _acf_field_gallery_items() {

        return [
            'key' => 'field_masonry_gallery_items',
            'label' => __( 'Gallery items', 'ldr' ),
            'name' => 'masonry_gallery_items',
            'type' => 'gallery',
            'instructions' => '',
            'required' => 0,
            'conditional_logic' => 0,
