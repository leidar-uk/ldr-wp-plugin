...
$service_id = (int) get_field( 'service_card_select' );
$cover_image_height = get_field( 'service_card_cover_image_height' );
$cover_image_overlay_color = get_field( 'service_card_cover_image_overlay_color' );
$card_min_height = get_field( 'service_card_min_height' );
$layout = get_field( 'service_card_layout' );
$is_column = ( ( $layout === 'column' ) || ( ! $layout ) ) ? true : false;
$card_class .= $is_column ? ' is-column' : ' is-row';
$card_hide_border = get_field( 'service_card_hide_border' );

$service_name = get_the_title( $service_id );
$service_hide_description = get_field( 'service_card_hide_description' );
$service_description = get_the_excerpt( $service_id );
$service_cover_image = get_the_post_thumbnail_url( $service_id );

$service_link = get_permalink( $service_id );
$service_link_text = get_field( 'service_card_link_text' );
...
$cover_image_container_style .= "background-image: url($service_cover_image);";
    ...
    <div class="<?php echo $card_class; ?>">
        ...
        <?php endif; ?>
                    ...
                    <div class="<?php echo $card_body_class; ?>">
                        <div>
                            <p class="h4 card-title mt-0<?php echo ( $service_description !== '' && ! $service_hide_description ) ? '' : ' mb-0'; ?>"><?php echo empty( $service_custom_name ) ? $service_name : $service_custom_name; ?></p>

                            <?php if( ! $service_hide_description ) : ?>
                                <p class="card-text mb-2"><?php echo $service_description; ?></p>
                            <?php endif; ?>
                        </div>

                        <a href="<?php echo $service_link?>" class="btn btn-ldr-primary mt-4 align-self-start"><?php echo $service_link_text !== '' ? $service_link_text : __( 'Find out more', 'ldr' ); ?></a>
                    </div>
