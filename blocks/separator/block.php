...
class Separator_Block {
    ...
    public function ldr_register_acf_block() {
        ...
        if( function_exists( 'acf_register_block_type' ) ) {
            ...
            acf_register_block_type( [
                'name'              => 'separator',
                'title'             => __( 'Separator', 'ldr' ),
                'description'       => __( 'A horizontal line.', 'ldr' ),
                'render_template'   => $this->block_dir . 'template.php',
                'category'          => 'custom',
                'icon'              => file_get_contents( $this->block_dir . 'images/separator.svg' ),
                'keywords'          => ['separator', 'line', 'divider'],
                'enqueue_assets'    => function() {
                    wp_enqueue_style( 'separator', $this->block_url . 'style.min.css', [], filemtime( $this->block_dir . 'style.min.css' ) );
                    // wp_enqueue_script( 'separator', $this->block_url . 'script.min.js', [], filemtime( $this->block_dir . 'script.min.js' ), true );
                }
            ] );
    ...
    // Bottom spacing
    protected function _acf_field_separator_bottom_spacing() {

        return [
            'key' => 'field_separator_bottom_spacing',
            'label' => '',
            'name' => 'separator_bottom_spacing',
            'type' => 'true_false',
            'instructions' => '',
            'required' => 0,
            'conditional_logic' => 0,
